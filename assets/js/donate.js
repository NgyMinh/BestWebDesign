...
function chuyenStepRa(stepEl, callback) {
	...
	// Đặt vị trí ban đầu
	stepEl.style.transition = "transform 0.5s";
	stepEl.style.transform = "translateX(0)";

	// Lướt ra ngoài về trái
	setTimeout(() => {
		stepEl.style.transform = "translateX(-100%)";
	}, 10);

	// Khi animation hoàn tất, ẩn bước và gọi callback
...
function chuyenStepVao(stepEl) {
	...
	// Đặt vị trí ban đầu ở ngoài bên phải
	stepEl.style.transition = "transform 0.5s";
	stepEl.style.transform = "translateX(100%)";
	stepEl.classList.remove("hidden");

	// Lướt vào từ phải về vị trí ban đầu
	setTimeout(() => {
		stepEl.style.transform = "translateX(0)";
	}, 10);

	// Dọn dẹp sau khi animation hoàn tất
	stepEl.addEventListener(
		"transitionend",
		() => {
			stepEl.style.transition = "";
