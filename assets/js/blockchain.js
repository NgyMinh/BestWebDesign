...
function quyDoiTienTe(token, soLuong, donVi) {
	...
	if (donVi === "VND") {
		// Hiển thị chuẩn tiền Việt: 1.234.567 VNĐ
		return new Intl.NumberFormat("vi-VN", {
			style: "currency",
			currency: "VND",
			minimumFractionDigits: 0, // bỏ số lẻ thập phân
			maximumFractionDigits: 0,
		}).format(ketQua);
	}

	// USD giữ nguyên format chuẩn
...
function khoiTaoSelect2TokenVaDonVi() {
	...
	`);
		...
		if ($unitSelect.length) {
			...
			$unitSelect.select2({
				placeholder: "Chọn loại tiền quy đổi",
				width: "100%",
				dropdownAutoWidth: true,
...
function chuyenBuocBlockchain(buoc) {
	...
	// Cập nhật giao diện stepper
	capNhatStepperBlockchain(buoc);

	// Nếu chuyển về bước nhỏ hơn hoặc cùng bước, không cần animation
	if (buoc < buocHienTaiBlockchain) {
		if (buocTruoc) {
			buocTruoc.classList.add("hidden");
			buocTruoc.style.transition = "";
			buocTruoc.style.transform = "";
			buocTruoc.style.opacity = "";
		}
		if (buocMoi) {
			buocMoi.classList.remove("hidden");
			buocMoi.style.transition = "";
	...
	// Nếu ở lại cùng bước thì bỏ qua
	if (buoc === buocHienTaiBlockchain) return;
...
function updateBlockchainStepper(step) {
	const steps = [document.getElementById("bc-step1"), document.getElementById("bc-step2"), document.getElementById("bc-step3")];

	// Tìm step đang hiển thị
	const currentStep = steps.find((el) => !el.classList.contains("hidden"));
	const nextStep = steps[step - 1]; // step truyền vào = 1,2,3

	if (currentStep === nextStep) return; // Không làm gì nếu cùng bước

	if (currentStep) {
		luotRaBlockchain(currentStep, () => {
			luotVaoBlockchain(nextStep);
		});
	} else {
		luotVaoBlockchain(nextStep);
	}
...
function hienThiXacNhanBlockchain() {
	const name = document.getElementById("bc-donor-name")?.value.trim() || "[N/A]";
	const email = document.getElementById("bc-donor-email")?.value.trim() || "[N/A]";
	const project = document.getElementById("bc-donation-project")?.value || "[N/A]";
	const token = document.getElementById("crypto-token")?.value || "[N/A]";
	const tokenAmount = document.getElementById("token-amount")?.value || "0";
	const converted = document.getElementById("converted-amount-display")?.value || "[N/A]";

	const walletAddress = "0x51C38ae80a40dFBa021C723D9ebB3a43b78B7A74";
	const txid = "BC-" + Math.random().toString(36).substring(2, 10).toUpperCase();

	const now = new Date();
	const dateStr = now.toLocaleString("vi-VN", { timeZone: "Asia/Ho_Chi_Minh" });

	// Đổ dữ liệu
	document.getElementById("bc-cf-name").innerText = name;
	document.getElementById("bc-cf-email").innerText = email;
	document.getElementById("bc-cf-project").innerText = project;

	// Gộp thành 1 dòng: "2 ETH"
	document.getElementById("bc-cf-crypto").innerText = `${tokenAmount} ${token}`;

	document.getElementById("bc-cf-converted").innerText = converted;
	document.getElementById("bc-cf-address").innerText = walletAddress;
	document.getElementById("bc-cf-txid").innerText = txid;
	document.getElementById("bc-cf-date").innerText = dateStr;
}
...
function capNhatHanQRVaSoTien() {
	const now = new Date();
	const expiry = new Date(now.getTime() + 15 * 60 * 1000); // +15 phút

	const gio = expiry.getHours().toString().padStart(2, "0");
	const phut = expiry.getMinutes().toString().padStart(2, "0");
	const ngay = expiry.getDate().toString().padStart(2, "0");
	const thang = (expiry.getMonth() + 1).toString().padStart(2, "0");
	const nam = expiry.getFullYear();

	const timeStr = `${gio}:${phut}, ${ngay}/${thang}/${nam}`;
	document.getElementById("qr-expiry-time").textContent = timeStr;

	// Cập nhật số tiền
	const soLuong = parseFloat(document.getElementById("token-amount").value) || 0;
	const token = document.getElementById("crypto-token").value || "";
	const donVi = document.getElementById("currency-unit").value || "";

	const quyDoi = quyDoiTienTe(token, soLuong, donVi);
	document.getElementById("qr-payment-amount").textContent = quyDoi;
}
